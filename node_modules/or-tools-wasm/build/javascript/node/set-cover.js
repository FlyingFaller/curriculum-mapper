// ../javascript/lib/set_cover_api.ts
...
var ConsistencyLevel = /* @__PURE__ */ ((ConsistencyLevel2) => {
  ConsistencyLevel2[ConsistencyLevel2["COST_AND_COVERAGE"] = 1] = "COST_AND_COVERAGE";
  ConsistencyLevel2[ConsistencyLevel2["FREE_AND_UNCOVERED"] = 2] = "FREE_AND_UNCOVERED";
  ConsistencyLevel2[ConsistencyLevel2["REDUNDANCY"] = 3] = "REDUNDANCY";
...
var setCoverModulePromise = null;
var setCoverModule = null;
...
var SetCoverModel = class {
  modelName = "SetCoverModel";
  costs = [];
  subsetElements = [];
  rowElements = [];
  validRows = false;
...
var SetCoverDecision = class {
  constructor(subsetIndex = 0, decisionValue = true) {
...
var SetCoverInvariant = class {
  ...
  compress_trace() {
    this.solutionTrace = this.selected.map((value, subset) => value ? new SetCoverDecision(subset, true) : null).filter((value) => value !== null);
  }
  load_solution(solution) {
    ...
    this.solutionTrace = solution.map((value, subset) => value ? new SetCoverDecision(subset, true) : null).filter((value) => value !== null);
    this.recompute();
  }
  check_consistency(_consistency) {
    this.recompute();
    return this.currentCoverage.length === this.currentModel.num_elements && this.selected.length === this.currentModel.num_subsets;
  }
...
var SetCoverSolutionGenerator = class {
...
var TrivialSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "trivial", "TrivialGenerator");
  }
};
var RandomSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "random", "RandomGenerator");
  }
};
var GreedySolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "greedy", "GreedyGenerator");
  }
};
var ElementDegreeSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "elementDegree", "ElementDegreeGenerator");
  }
};
var LazyElementDegreeSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "lazyElementDegree", "LazyElementDegreeGenerator");
  }
};
var SteepestSearch = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "steepest", "SteepestSearch");
  }
};
var GuidedLocalSearch = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
...
var GuidedTabuSearch = class extends SetCoverSolutionGenerator {
  lagrangianFactor = 100;
  epsilon = 1e-6;
  penaltyFactor = 0.3;
  tabuListSize = 17;
  constructor(invariant) {
...
function clear_random_subsets(numSubsetsOrFocus, invariantOrNumSubsets, maybeInvariant) {
  const invariant = typeof numSubsetsOrFocus === "number" ? invariantOrNumSubsets : maybeInvariant;
  const numSubsets = typeof numSubsetsOrFocus === "number" ? numSubsetsOrFocus : invariantOrNumSubsets;
  ...
  for (let subset = 0; subset < selected.length && chosen.length < numSubsets; subset++) {
    if (selected[subset]) {
      invariant.deselect(subset, 1 /* COST_AND_COVERAGE */);
      chosen.push(subset);
    }
  }
  return chosen;
}
var clear_most_covered_elements = clear_random_subsets;
...
var read_orlib_scp = read_set_cover_proto;
var read_orlib_rail = read_set_cover_proto;
var read_fimi_dat = read_set_cover_proto;
var write_orlib_scp = write_set_cover_proto;
var write_orlib_rail = write_set_cover_proto;
var write_set_cover_solution_text = write_set_cover_solution_proto;
var read_set_cover_solution_text = read_set_cover_solution_proto;

// ../javascript/lib/set-cover.ts
import {
